"""Journal table JSON builder with stdlib-only imports.

Deployed to HA /share for delete_journal_entries.py (runs outside addon venv).
Keep in sync with journal aggregation rules in this module.
"""
...
def build_journal_table_payload(journal_path: Path) -> dict[str, Any]:
    """Build the HA journal_table.json payload from a JSONL journal path (stdlib-only)."""
