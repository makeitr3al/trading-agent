...
5. **Observability:** `app.journal` appends structured journal entries. Multi-market scan (`scripts/multi_market_scan.py`) may emit `scan_cycle_phase` on cycle rows (`dry_run` | `execute` | `scan_failed`); failures before `run_app_cycle` use `scan_failed` with details in `notes`. In `DATA_SOURCE=live`, `AssetRegistry.validate_scan_asset_for_hyperliquid_fetch` preflights each market before candle fetch (crypto perp names; HIP-3 uses dex-qualified `xyz:…` strings for Hyperliquid `coin`, validated against the registry). `utils/journal_table.py` may dedupe paired `dry_run`/`execute` cycle rows for the same batch key (execute wins).
