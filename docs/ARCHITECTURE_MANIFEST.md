...
- Execution only through the **Propr** API; primary market data from **Hyperliquid** (REST candles/L2; optional public WebSocket `trades` for virtual-stop watching).
...
|--------|------|----------------|
| Runtime / orchestration | `app/` | One cycle: call strategy, guards, broker I/O, journal. Per-cycle helpers (sizing, slot counts, etc.) live in `app/app_cycle_helpers.py`; `app/trading_app.py` remains the phase pipeline and public `run_app_cycle` entry. |
...
| Data access | `data/providers/` | `CandleDataProvider` implementations (live, golden, historical). Optional public Hyperliquid **trades** WebSocket (`hyperliquid_ws.py`) for virtual-stop watching (not candle REST). |
| Backtesting | `backtest/` | Offline simulation and screening: `daily_universe.py`, Propr-whitelist profitability screener (`screener.py`, `metrics.py`, public catalog via `propr_exchange_assets.py`). Reuses `strategy/` logic; **no** Propr execution or submit. |
...
**Propr integration:** All authenticated HTTP/SDK calls to Propr go through **`broker/`** (`propr_client`, `order_service`, `state_sync`, `execution`, etc.). The published **`propr_sdk`** Python package is imported only from **`broker/propr_sdk.py`** — it is not a second app-layer tree.
...
- `backtest/` may import `strategy`, `config`, `models`, `data/providers`, and optionally `broker.asset_registry` for asset-type classification. It must **not** import `app` or submit paths under `broker.execution` / `broker.order_service`.
...
- Do not add a parallel in-repo package for Propr **authenticated** calls; extend **`broker/`** (and **`propr_sdk`** via `broker/propr_sdk.py` if the SDK surface changes).
- **Known exception (read-only research):** `backtest/propr_exchange_assets.py` may GET the **public** exchange-assets catalog (`/v1/exchange-assets/config/hyperliquid`, no API key) for whitelist screening only. No orders, positions, or credentials.
...
- **Offline backtests:** `scripts/backtest_daily_universe.py` (raw HL universe) and `scripts/backtest_market_screener.py` (Propr-whitelist profitability screener with GO/NO_GO; merge via `scripts/backtest_screener_merge.py`). No submit.
...
2. **Strategy:** `strategy.engine.run_agent_cycle` runs signal/regime/decision logic and produces a `StrategyRunResult` (and order intent shapes as defined by models/strategy). For **open countertrend** positions, the managed take-profit middle-band level is taken from the same **closed-bar-aligned** Bollinger frame as signal detection (`bollinger_sig` in `strategy_runner.run_strategy_cycle` — excludes the still-forming last candle until interval-close), not from a live-only “forming bar” Bollinger row.
3. **State:** `broker.state_sync.sync_agent_state_from_propr` refreshes `AgentState` from Propr when the cycle uses live broker state. Implementation is split across `state_sync` (orchestration + `AgentState` build), `propr_order_position_map`, `propr_payload_parse`, and `utils.propr_response` for duplicated key/id logic — import **through** `broker.state_sync` (or `execution`’s existing imports) unless you are editing those internals. When called with `symbol=None`, `build_agent_state_from_propr_data` runs in an **account-wide observability mode**: it does **not raise** on multiple open positions / pending entries, returns only **account aggregates** (`account_open_positions_count`, `account_open_entry_orders_count`, `account_unrealized_pnl`), and leaves decision-driving fields (`active_trade`, `pending_order`, ids) as `None`.
4. **Execution:** Guards in `app` + `broker.execution` / `broker.order_service` submit, replace, or cancel on Propr as policy allows. **New pending entries** go as one **`create_orders` bracket batch** (entry + `stop_market` + `take_profit_limit` under a shared `orderGroupId` via `submit_bracket_entry_with_exits` → `ProprClient.create_orders_batch_raw`), not entry-only standalone stops.
...
- **Positions:** `quantity == 0` positions are not treated as active trades after sync (lenient open-row detection also filters these).
- **No symbol pyramiding:** While any **lenient open** broker position row exists for the scoped symbol (`AgentState.has_open_broker_position_for_symbol`) **or** a fresh ``get_positions`` probe finds one, `submit_agent_order_if_allowed`, `safe_replace_pending_order`, and `_phase_pending_trigger` refuse **new entry brackets** with `skip_reason` / journal **`submit blocked: open position present at broker for symbol`** or **`open position present at broker for symbol`** (trigger phase). Propr merges same-asset same-side fills into one position; this guard blocks accidental size increases when strict `Trade` mapping fails (e.g. bare position without SL/TP on the REST snapshot).
- **Entry SL/TP (bracket):** Pending-entry execution uses Propr **batching** (SDK `create_orders` with `order_group_id` / REST top-level `orderGroupId`): one entry row plus reduce-only exit rows (`stop_market`, `take_profit_limit`) in `broker/order_service` (`build_bracket_submission_previews` / `submit_bracket_entry_with_exits` → `ProprClient.create_orders_batch_raw`), invoked from `broker/execution` (`submit_agent_order_if_allowed`, `safe_replace_pending_order`). This satisfies conditional-order grouping rules (see [propr-docs: batching](https://github.com/XBorgLabs/propr-docs/blob/main/docs/api.md#batching-and-conditional-order-rules)).
- **Stop entries:** Propr treats **only** `market`/`limit` as entry orders in batching rules. Stop-entry intents (`BUY_STOP`/`SELL_STOP` → API `stop_limit`) are conditional orders and are **blocked pre-submit** in `broker/execution` with a clear `skip_reason` to avoid Propr `13056` errors.
- **Virtual stop trigger:** Stop-entry intents are held internally as `AgentState.pending_order`. Broker-less `BUY_STOP`/`SELL_STOP` survive Propr state sync (`build_agent_state_from_propr_data` / `_resolve_local_virtual_stop_pending`) until the strategy clears them. Submit is owned by `app/armed_stop_submit.execute_armed_stop_market_bracket` (market entry + exits). Modes via `TREND_STOP_TRIGGER_MODE`: `last_candle` (default; OHLC touch in `_phase_pending_trigger` and daemon poll; bar-boundary catch-up on the frozen entry before strategy refresh), `ws` (Hyperliquid trade prints in `scripts/trigger_polling_daemon.py` via `data/providers/hyperliquid_ws.py`, same bar-boundary catch-up; OHLC fallback if WS down), or `disabled`. Optional gap skip: `TREND_STOP_MAX_GAP_R_RATIO` (default `0.5`) disarms the lifecycle when the print/touch is too far past entry. Live cycles pass `synthesize_local_fills=False` into `run_agent_cycle` so touched pendings do not become local `active_trade` without a Propr fill. Intent seeds for armed stops use `signal_lifecycle_id` (no wall-clock) when `PROPR_STABLE_INTENT_ID=YES`.
- **Journal bar dedupe:** With interval runners, journaling may run many times per bar. `AgentState.last_journaled_signal_bar_ts` is used by `app/trading_app` to skip emitting identical `cycle`/`order` rows within the same closed “signal bar” unless a meaningful state change occurred (submit/replace/close/exit update).
- **State sync recovery:** On live sync, `sync_agent_state_from_propr` merges exit levels from open orders into position rows (`enrich_positions_payload_with_exit_levels_from_orders`) before mapping. If `map_propr_position_to_internal` still yields **no** `Trade` for the symbol’s open position (strict mapper requires `side`, `entry`, and **`stop_loss`**; `take_profit` may be absent), and there is **exactly one** matching open position row, `build_agent_state_from_propr_data` → `_resolve_active_trade_with_previous_fallback` may set `active_trade` by **synthesizing** SL/TP from **`previous_state.pending_order`** or **`previous_state.active_trade`** (direction must match the position) so exit management can attach or refresh SL/TP. Independently, **`AgentState.has_open_broker_position_for_symbol`** is set whenever **any** lenient open position row matches the normalized symbol (even when strict mapping fails), so execution gates and operators can still see broker exposure on that symbol.
- **API probe note:** A **standalone** `BUY_STOP`/`SELL_STOP` submit (single order, no group) may still fail with `conditional_order_requires_position_or_group` (HTTP 400 / code 13056). That is distinct from the agent’s bracket path; `scripts/propr_order_types_test.py` probes raw order types and may skip on 13056.
...
**Version:** Document creation tracks repository state; bump or date this section when making substantive contract changes. **2026-04:** Orchestration split documented — `app/app_cycle_helpers.py`, `utils/propr_response.py`, `broker/propr_payload_parse.py`, `broker/propr_order_position_map.py`, thinner `broker/state_sync.py` facade; dependency rules in §2.2 unchanged. **2026-04 (later):** Execution reconciliation (`SubmitAgentOrderResult.existing_external_order_id`), optional `PROPR_STABLE_INTENT_ID` + `derive_stable_intent_id`, partial-fill pending omission when a position exists for the symbol. **2026-04 (data):** `validate_data_batch` + `min_strategy_candle_count` — enforced at provider return; golden fixtures use UTC-aware candle timestamps. **2026-04 (multi-market):** Live HL perp coin preflight via asset registry; scan journal `scan_cycle_phase` including `scan_failed`; journal-table dedupe of `dry_run`/`execute` cycle rows; per-asset loop failures non-fatal (remaining markets still scanned). **2026-05:** §5 state-sync recovery — enrichment-before-map + synthetic `active_trade` preconditions (`stop_loss` required for mapper, single position, direction-matched `previous_state`); §4 execution — explicit `create_orders` / `create_orders_batch_raw` wording. **2026-05 (later):** §5 — `has_open_broker_position_for_symbol` + per-submit `get_positions` probe; no symbol pyramiding when strict `Trade` mapping fails. **2026-09:** §2 Backtesting layer — Market Profitability Screener (`scripts/backtest_market_screener.py`, `backtest/screener.py`, public catalog `backtest/propr_exchange_assets.py`, merge `scripts/backtest_screener_merge.py`); §2.2/§2.3 backtest dependency + public-catalog exception. **2026-09 (virtual stop):** §1/§5 — HL trades WebSocket watcher (`data/providers/hyperliquid_ws.py`), `app/armed_stop_submit.py`, sync preserves broker-less stops, `TREND_STOP_TRIGGER_MODE=ws`, bar-boundary catch-up, gap guard, live `synthesize_local_fills=False`.
