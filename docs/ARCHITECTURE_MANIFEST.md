...
| Backtesting | `backtest/` | Offline simulation and screening (e.g. daily universe backtest) reusing `strategy/` logic; no Propr execution or submit. |
...
- **Offline backtests:** `scripts/backtest_daily_universe.py` — Hyperliquid 1D universe screening (no submit).
...
- **Virtual stop trigger (trend):** Trend stop-entry intents are held internally as `AgentState.pending_order` (`OrderType.BUY_STOP` / `SELL_STOP`). When `TREND_STOP_TRIGGER_MODE=last_candle` (default) and the live provider’s **last candle** high/low touches the trigger price, `app/trading_app._phase_pending_trigger` submits a **market-entry bracket** batch via `ProprOrderService.submit_market_entry_bracket_with_exits` (entry + `stop_market` + `take_profit_limit` under a shared `orderGroupId`). Resting stop-entry submits remain blocked in `broker/execution`.
