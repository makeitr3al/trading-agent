...
- `scheduling_enabled`
- `schedule_time`
- `trigger_polling_enabled` (optional; wenn `true`, erzwingt der Runner `RUNNER_MODE=interval` mit `RUNNER_INTERVAL_SECONDS` (Default 60) und Scheduling wird im Panel deaktiviert)
...
**Save-argv-Vertrag (wichtig):** `shell_command.trading_agent_save_operator_config_haos` (siehe `home_assistant_package_haos_addon.yaml.example`) fuehrt `ha_save_operator_config.py` mit **neun** Positionsargumenten aus (`MODE`, `ENV`, `LEVERAGE`, `MARKETS`, `SCHED_BOOL`, `SCHED_TIME`, `CHALLENGE_ATTEMPT_ID`, `PUSH_BOOL`, `TRIGGER_POLLING_BOOL`). Jeder `scripts.yaml`-Eintrag, der diesen `shell_command` aufruft — also auch **Preflight**, **Beta-Write** und **Scharf-Lauf** — muss im `data:`-Block **`challenge_attempt_id`**, **`push_enabled`** und **`trigger_polling_enabled`** setzen (letzteres optional, Default `false`). Wenn diese Keys fehlen, kann `operator_config.json` zwar teils aktualisiert werden, aber **`challenge_attempt_id`** und Add-on-Logs zu `PROPR_CHALLENGE_*` passen nicht mehr zur Referenz. **`script.trading_agent_load_current_config_haos`** muss den Helper `input_text.trading_agent_challenge_id` vorzugsweise aus **`challenge_attempt_id`** (Fallback `challenge_id`) befuellen und idealerweise `wait_template` + Datei-Fallback aus `home_assistant_scripts_haos_addon.yaml.example` nutzen.

## Verifikation (nach Script-/Paket-Update)

1. **Developer Tools → Dienste:** `shell_command.trading_agent_save_operator_config_haos` erscheint in der Liste — **nicht** mit `data: {}` testen: ohne `data` bleiben die Jinja-Templates (z. B. `"{{ mode }}"`, `"{{ leverage }}"`) leer; nur `trigger_polling_enabled` faellt per Default auf `'false'`. Das Referenz-Skript **schreibt dann nicht** (ab Add-on **0.6.11**). Zum manuellen Speichern **`script.trading_agent_save_current_config_haos`** ausfuehren.
