...
def test_maps_buy_limit_order_to_sdk_create_order_parameters(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_maps_buy_stop_order_to_sdk_stop_limit_parameters(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("broker.order_service.generate_intent_id", lambda: "ulid-fixed")
...
def test_maps_sell_stop_order_to_sdk_stop_limit_parameters_with_short_position_side(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("broker.order_service.generate_intent_id", lambda: "ulid-fixed")
...
def test_includes_asset_base_quote_time_in_force_and_execution_flags(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("broker.order_service.generate_intent_id", lambda: "ulid-fixed")
    params = map_internal_order_to_propr_payload(_make_order(OrderType.BUY_LIMIT), "BTC/USDC")
...
def test_submit_pending_order_calls_sdk_adapter_with_mapped_parameters(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert params["order_type"] == "stop_limit"
    assert params["asset"] == "BTC/USDC"
    assert "intent_id" not in params
    assert response["data"][0]["orderId"] == "urn:prp-order:123"


def test_build_sdk_create_order_params_drops_intent_id_for_sdk_call() -> None:
